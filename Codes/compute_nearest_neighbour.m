function[nearest_neighbour]=compute_nearest_neighbour(candidate_points,given_points)
...
%an N1 x 3 matrix of candidate_points
%an N2 x 3 matrix of given_points (N1 and N2 can be different)

%OUTPUT:

%an N2 x 1 matrix (column) with the numbers of the nearest neighbour points to each candidate point.
%(The number of a row in the nearest neighbour matrix is equivalent to the number of
%the given point in the candidate points matrix)
...
DistanceX=repmat(candidate_points(:,1),[1,length(given_points)])-repmat(given_points(1,:),[length(candidate_points),1]);
DistanceY=repmat(candidate_points(:,2),[1,length(given_points)])-repmat(given_points(2,:),[length(candidate_points),1]);
DistanceZ=repmat(candidate_points(:,3),[1,length(given_points)])-repmat(given_points(3,:),[length(candidate_points),1]);
