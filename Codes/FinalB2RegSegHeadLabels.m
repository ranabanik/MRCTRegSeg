...
for nn = 1:length(stat)
    ...
    if zz > size(lab,3)/2 % avoid taking teeths
        [~, bi] = regionGrowing(ct, [xx, yy, zz], 1500, 50); % region grow (1600 could be changed based on whether the result bi include skull)
...
bi = (lab==0&ct>300&ct<1600&ct_grad<30000&mr>0); % ct<2000; 300-1500 % value 400 gets rid of peripheral ct components of machine
% mr > 0 added. 
...
%     nii.img = lab;
%     save_untouch_nii_gz(nii, 'afterfullFatMRLab.nii.gz');

nii2 = load_untouch_nii_gz([subID, '_MRstd_2_MNI.nii.gz']); % different sizes of norm so nii2 instead of nii
mr_norm = nii2.img;
% clear nii
...
v = zeros(size(mr_norm));
v(eye2MRnorm==1) = mr_norm(eye2MRnorm==1);
v_flat = v(v~=0);
